...
import type { Reactive, DispatchingStore, Store } from './types'

export function useReactiveMap<K, V>(source: ReactiveMap<K, V>) {
  return useReactive(source, [Array.from(source.entries()), source])
}
